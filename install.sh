...
# All checks passed:
# Copy the main .env file over to the Laravel app directory
...
# Grab the app key generated by the script above and display in completion message
app_line=$(grep '^APP_KEY=' src/.env)
unique_app_key=${app_line#APP_KEY=}
